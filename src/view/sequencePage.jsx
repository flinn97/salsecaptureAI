...
export default class SequencePage extends GetComponentsFromUrl {
  ...
  async componentDidMount() {
    ...
    if (!sequence) {
      sequence = await this.operationsFactory.prepare({
        prepare: { type: "sequence" },
        clean: true,
      });
      sequence = sequence[0];
    } else {
  ...
  getStepJson() {
    ...
    let newStep = {
      type: "step",
      sequenceId: this.propsState.currentSequence?.getJson()._id,
      step: list.length,
    };
    return newStep;
  }
        ...
        <div className="mobile-container">
          <div className="title-bar">
            <div className="title-left">
              ...
              {this.propsState.currentSequence?.getJson().name === "" ||
              this.state.edit ? (
                <div className="name-a-sequence">
                  <ParentFormComponent
          ...
          <div className="switch-tab">
            <div className="row row-space-around">
              <div
                onClick={() => {
                  this.setState({ spState: "assignedProspects" });
                }}
                className={
              ...
              <div
                onClick={() => {
                  this.setState({ spState: "sequenceSteps" });
                }}
                className={
                  this.state.spState === "assignedProspects"
                    ? "switch-tab-btn"
                    : "switch-tab-btn active"
                }
              >
          ...
          {this.state.spState === "sequenceSteps" ? (
            <div className="icon-list">
              <MapComponent
                ...
                filter={{
                  search: this.propsState.currentSequence?.getJson()._id,
              ...
              <div className="icon-row">
                <div className="svg-last"></div>
              </div>
            </div>
          ) : (
            <div
              className="layoutColumn conversation-container"
              style={{ padding: "20px" }}
            >
              <MapComponent
                name="contact"
                ...
                cells={[
                  {
                    type: "custom",
                    custom: AssignedProspectsCustom,
                    wrapperClass: "none",
                  },
                ]}
              />
  ...
  render() {
    return (
      <div
        className={[
          this.props.pageClass || this.state.defaultClass,
          "sequence-margin",
        ].join(" ")}
      >
